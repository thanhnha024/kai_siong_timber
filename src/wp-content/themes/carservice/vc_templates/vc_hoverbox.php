...
if ( $atts['hover_add_button'] ) {
	$button_atts = vc_map_integrate_parse_atts( 'vc_hoverbox', 'vc_btn', $atts, 'hover_btn_' );
	$params = "";
	foreach ($button_atts as $key => $value) {
	  $params .= $key . '="' . esc_attr($value) . '" ';
	}
	$button = do_shortcode('<div class="margin-top-20 padding-bottom-20">[vc_btn ' . $params . ']</div>');
}
$template = <<<HTML
